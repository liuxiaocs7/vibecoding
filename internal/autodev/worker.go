...
func (r *Runner) run(ctx context.Context, jobID string) error {
	...
	if issue.HasSubRequirements() {
		if err := r.developSubs(ctx, job, issue, repos, cfg, progress); err != nil {
			return err
		}
	} else {
		if err := r.developOne(ctx, job, issue, repos, cfg, issue.DevSpec, issue.Title, issue.Description, "", 40, 90, true, progress); err != nil {
			return err
		}
	}
	...
	pr := &model.PRInfo{
		...
		Status:      "open",
		DiffStats:   r.collectDiffStats(repos),
	}
...
func (r *Runner) developSubs(
	...
	if reworkID != "" {
		...
		if !found {
			return fmt.Errorf("rework sub-requirement %s not found", reworkID)
		}
	} else {
	...
	n := len(indices)
	if n == 0 {
		return fmt.Errorf("no sub-requirements to develop")
	}

	for k, idx := range indices {
		sub := &issue.SubRequirements[idx]
		if sub.DevSpec == nil {
			return fmt.Errorf("sub-requirement %q has no Dev Spec", sub.Title)
		}
		...
		if err := r.developOne(ctx, job, issue, repos, cfg, sub.DevSpec, sub.Title, firstNonEmpty(sub.Description, issue.Description), extra.String(), pStart, pEnd, k == n-1, progress); err != nil {
			sub.Status = model.SubReqFailed
			issue.UpdatedAt = model.NowISO()
			_ = r.Store.UpsertIssue(*issue)
			return fmt.Errorf("sub-requirement %q: %w", sub.Title, err)
		}
		...
		// reload in case appendLog overwrote
		...
		if target := issue.SubByID(sub.ID); target != nil {
			...
			if target.DevSpec != nil && issue.SubByID(sub.ID).DevSpec != nil {
				// keep generated file changes if developOne persisted them
			}
		}
		issue.CurrentSubID = sub.ID
		issue.UpdatedAt = model.NowISO()
...
func (r *Runner) developOne(
	...
	if spec == nil {
		return fmt.Errorf("missing Dev Spec")
	}
	span := pEnd - pStart
	if span < 8 {
		span = 8
	...
	fileChanges := spec.FileChanges
	snaps, err := repocontext.Collect(repos, fileChanges)
	if err != nil {
		return err
	}
	_ = r.appendLog(job, "analyzing", fmt.Sprintf("Collected context from %d repo(s) for %s", len(snaps), title), "")

	if err := progress(pCode, "coding", fmt.Sprintf("Generating code for %s...", title)); err != nil {
		return err
	}
	changes, err := r.generateCode(ctx, cfg, title, desc, spec, snaps, extra)
	if err != nil {
		return err
	}
	totalApplied := 0
	for _, repo := range repos {
		...
		totalApplied += n
		_ = r.appendLog(job, "coding", fmt.Sprintf("[%s] applied %d file change(s) (%s)", repo.Name, n, title), "")
	}
	if totalApplied == 0 {
	...
	if len(changes) > 0 {
		...
		if issue.HasSubRequirements() {
			...
			if target := issue.SubByID(issue.CurrentSubID); target != nil && target.DevSpec != nil {
				target.DevSpec.FileChanges = changes
				target.DevSpec.UpdatedAt = spec.UpdatedAt
			}
		} else if issue.DevSpec != nil {
			issue.DevSpec.FileChanges = changes
...
func (r *Runner) generateCode(ctx context.Context, cfg model.ModelConfig, title, desc string, spec *model.DevSpec, snaps []repocontext.RepoSnapshot, extra string) ([]model.SpecFileChange, error) {
...
- If extra instructions mention a sub-requirement, implement only that slice of work.`
	...
	user := fmt.Sprintf("Issue: %s\nDescription: %s\n\n%s\nDevSpec JSON:\n%s\n\nRepository context:\n%s\n\nProduce the fileChanges JSON now.",
		title, desc, extra, string(specJSON), repocontext.FormatForPrompt(snaps))
